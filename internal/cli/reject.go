...
func newRejectCmd(deps *Dependencies) *cobra.Command {
	var feedback string
	var jsonOutput bool

	cmd := &cobra.Command{
		Use:   "reject <task-id>",
		Short: "Reject the pending gate and re-run the stage with feedback.",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !jsonOutput {
				fmt.Printf("Rejecting gate for task %s...\n", taskID)
			}

			task, err = eng.Reject(ctx, taskID, feedback)
			if err != nil {
				return err
			}
	...
	cmd.Flags().StringVar(&feedback, "feedback", "", "Rejection feedback (required).")
